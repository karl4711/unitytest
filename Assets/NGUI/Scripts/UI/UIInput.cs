...
public class UIInput : MonoBehaviour
{
	...
	protected char Validate (string text, int pos, char ch)
	{
        ...
        if (validation == Validation.Account)
        {
            // added by zhangyisong
            if (ch >= 'A' && ch <= 'Z') return ch;  
            if (ch >= 'a' && ch <= 'z') return ch;  
            if (ch >= '0' && ch <= '9') return ch;
            if (ch == '_') return ch;
        }
        else if (validation == Validation.PhoneNum)
        {
            // added by zhangyisong
            if (ch >= '0' && ch <= '9') return ch;
        }
        ...
        else if (validation == Validation.NickName)
        {
            if (Getstrlen(text) >= characterLimit) return (char)0;
            // 仅允许输入中英文以及数字下划线字符;
            if (char.IsLetter(ch)) return ch;
            if (char.IsNumber(ch)) return ch;
            if (ch == '_') return ch;
            Regex r = new Regex("^[\u4e00-\u9fa5]+$");
            if (r.IsMatch(ch.ToString())) return ch;
        }
		return (char)0;
	}
    ...
    int Getstrlen(string str)
    {
        ...
        {
            Regex r = new Regex("^[\u4e00-\u9fa5]+$");
            if (r.IsMatch(str[i].ToString())) len++;
            len++;
        }
        return len;
    }
