...
public class UIPanel : UIRect
{
    ...
    public delegate void RenderQueueDeleg();

    public RenderQueueDeleg renderQueueDeleg;

	static public List<UIPanel> list = new List<UIPanel>();

    //Unity 鐨凷priteRender鍜岀矑瀛愮郴缁熷姞鍏ュ埌NGUIRenderQueue鎺掑簭;
    List<RenderQueueDeleg> addList = new List<RenderQueueDeleg>();
    ...
    //澶栭儴鍔犲叆NGUI鎺掑簭鐨勭墿浣擄紝鍔犲叆addList
    public void AddToRenderSort(RenderQueueDeleg addToNGUI)
    {
        if(addList.Contains(addToNGUI))
        {
            return;
        }
        addList.Add(addToNGUI);
    }

    //浠巃ddList涓?Щ闄Ｌ
    public void RemoveRenderSort(RenderQueueDeleg addToNGUI)
    {
        if(addList.Contains(addToNGUI))
        {
            addList.Remove(addToNGUI);
        }
        else
        {
            return;
